def handle_api(headers):
    return {"status": 200, "response": {'response': 'API server running', 'headers': headers}}

def handle_info(query_params, headers):
    response = {"status": 200, "response": {'response': 'Info API', 'headers': headers}}
...
def handle_info_detail(id, query_params, headers):
    response = {"status": 200, "response": {'response': f'Selected user {id}', 'headers': headers}}
    check = query_params.get('check')
    if check is not None:
...
def handle_types(id, active, parent, query_params, headers):
    response = {"status": 200, "response": {'response': f'Selected type {id}, active {active}, parent {parent}', 'headers': headers}}
    check = query_params.get('check')
    if check is not None:
        response['check'] = check
    expanded = query_params.get('expanded')
...
def post_info(body, query_params, headers):
    response = {"status": 200, "response": {'body': body, 'params': query_params, 'headers': headers}}
    return response

def put_info(id, body, query_params, headers):
    response = {"status": 200, "response": {'body': body, 'element': id, 'params': query_params, 'headers': headers}}
    return response
